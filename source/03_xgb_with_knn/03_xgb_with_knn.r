...
for(i in 1:200){
	...
	train_knn_cos <- read.csv("../../add_features/KNN_cos_train.csv", stringsAsFactors = F)
	test_knn_cos <- read.csv("../../add_features/KNN_cos_test.csv", stringsAsFactors = F)

	train <- merge(train, train_knn_cos, by = "ID")
	test <- merge(test, test_knn_cos, by = "ID")

	## KNN euqlid ----------

	train_euclid_cos <- read.csv("../../add_features/KNN_euclid_train.csv", stringsAsFactors = F)
	test_euclid_cos <- read.csv("../../add_features/KNN_euclid_test.csv", stringsAsFactors = F)

	train <- merge(train, train_euclid_cos, by = "ID")
	test <- merge(test, test_euclid_cos, by = "ID")
